...
import { ShieldAlert, Search, Calendar, RefreshCcw } from 'lucide-react';
...
export function OperationsSupervision() {
    ...
    // Date Range Filter (Default: Last 60 Days)
    ...
    useEffect(() => {
        // --- CLEANUP LOCAL STORAGE (CRITICAL FIX) ---
        // Ensure we remove any old large caches that might be stuck
        try {
        ...
        loadSupervisionInitial();
    }, [dateRange]);

    const loadSupervisionInitial = async () => {
        setLoading(true);
        try {
            // 1. Load users first
            ...
            // 2. Load INITIAL 50 records (Fast load)
            ...
            if (!error && data) {
                setAllProcesses(data as any[]);

                // 3. Trigger Background Load for the rest (in a timeout to let UI render)
                setTimeout(() => {
                    loadSupervisionBackground();
                }, 100);
            }
        } catch (err) {
            console.error(err);
    ...
    const loadSupervisionBackground = async () => {
        try {
            // Load EVERYTHING in the range (replacing the initial 50 with full set)
            const { data, error } = await supabase
                .from('workflow_processes')
                .select(`
                    id, reference_id, created_at, title, process_type, status, metadata, escalation_level,
            ...
            if (!error && data) {
                // Optimization: Only update if we have more data
                setAllProcesses(prev => {
                    if (data.length > prev.length) return data as any[];
                    return prev;
                });
            }
        } catch (err) {
    ...
    return (
        <div className="space-y-6">
            <OperationsHeader
                ...
                onSyncComplete={() => loadSupervisionInitial()}
            />
            ...
            <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden animate-in fade-in duration-500 shadow-sm">
                <div className="p-6 border-b border-zinc-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex flex-col gap-1">
                        ...
                        {/* Date Range Display / Control */}
                        <div className="flex items-center gap-2 text-xs text-zinc-500 font-medium bg-zinc-50 px-3 py-1.5 rounded-lg border border-zinc-100 w-fit">
                            ...
                            <input
                                ...
                                className="bg-transparent border-none p-0 text-xs focus:ring-0 text-zinc-700 font-bold cursor-pointer w-24"
                            />
                            ...
                            <input
                                ...
                                className="bg-transparent border-none p-0 text-xs focus:ring-0 text-zinc-700 font-bold cursor-pointer w-24"
                            />
                            {loading && <RefreshCcw size={12} className="animate-spin text-blue-600 ml-2" />}
                        </div>
                    </div>
                    ...
                    <div className="relative w-full md:w-auto">
                        ...
                        <input
                            ...
                            className="bg-white border border-zinc-200 rounded-xl py-2 pl-9 pr-4 text-xs font-medium outline-none focus:border-zinc-400 focus:ring-4 focus:ring-zinc-50 transition-all w-full md:w-64 placeholder:text-zinc-300 text-zinc-700"
                        />
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-zinc-100">
                            {/* Only show Skeleton if we have ZERO data (no cache, no fresh) and we are loading */}
                            {loading && allProcesses.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="p-12 text-center text-zinc-400 text-xs font-medium animate-pulse">
                                        Cargando datos operativos (Instalando caché)...
                                    </td>
                                </tr>
